document.addEventListener('DOMContentLoaded', () => {
    ...
    // Load classes data once at startup
    async function loadClassesData() {
        try {
            // console.log('Loading classes data...');
            const response = await fetch('classes.json');
            if (!response.ok) {
                // console.warn('Failed to load classes.json:', response.status, response.statusText);
                return;
            }
    ...
    function findClassInfo(groupName) {
        // Normalize the incoming group name
        const normalizedGroupName = normalizeWhitespace(groupName);
        
        // Look for exact match first
        if (classesData[groupName]) {
            return classesData[groupName];
        }
        
        // Try normalized match
        if (classesData[normalizedGroupName]) {
            return classesData[normalizedGroupName];
        }
        
        // Search through all keys for a normalized match
        for (const key of Object.keys(classesData)) {
            if (normalizeWhitespace(key) === normalizedGroupName) {
    ...
    function setOYL(sheet, groupName) {
        // console.log(`setOYL called for group: "${groupName}"`);
        const classInfo = findClassInfo(groupName);
        // console.log(`Found classInfo:`, classInfo);
        if (classInfo && classInfo.programLeader) {
            // console.log(`Setting I3 to: "${classInfo.programLeader}"`);
            sheet.getCell('I3').value = classInfo.programLeader;
            sheet.getCell('I3').font = { size: 9, name: 'Calibri' };
            // console.log(`I3 value after set:`, sheet.getCell('I3').value);
        } else {
            // console.log(`No programLeader found, clearing I3`);
            sheet.getCell('I3').value = '';
        }
    }

    function setRoomTime(sheet, groupName) {
        // console.log(`setRoomTime called for group: "${groupName}"`);
        const classInfo = findClassInfo(groupName);
        // console.log(`Found classInfo:`, classInfo);
        if (classInfo && classInfo.room && classInfo.startTime && classInfo.endTime) {
            const roomTimeString = `${classInfo.room}/${classInfo.startTime} - ${classInfo.endTime}`;
            // console.log(`Setting N3 to: "${roomTimeString}"`);
            sheet.getCell('N3').value = roomTimeString;
            sheet.getCell('N3').font = { size: 9, name: 'Calibri' };
            // console.log(`N3 value after set:`, sheet.getCell('N3').value);
        } else {
            // console.log(`Missing room/time data, clearing N3`);
            sheet.getCell('N3').value = '';
        }
    }
    ...
    async function generateReport(groupName, attendanceData) {
        ...
        try {
            ...
            for (let sheetIndex = 0; sheetIndex < totalSheetsNeeded; sheetIndex++) {
                ...
                //Set OYL
                setOYL(sheet, groupName);

                // set room and time
                setRoomTime(sheet, groupName)
